...
`Reposter` — это автоматизированный инструмент для синхронизации и репостинга контента между социальными сетями VK.ru и Telegram. Приложение периодически проверяет новые публикации в указанных источниках VK, обрабатывает их (включая загрузку медиафайлов с помощью yt-dlp) и публикует в целевые каналы Telegram. Поддерживается гибкая настройка правил репостинга и параметров загрузки.
...
1. **Создайте Standalone-приложение:**
    * Перейдите в раздел [Мои приложения VK](https://vk.ru/apps?act=manage).
